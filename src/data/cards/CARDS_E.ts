...
export const CARDS_E: Record<string, Card> = {
  'E-01': {
    ...
    1: {
      type: 'Z',
      label: '找資安公司來，把硬碟直接物理銷毀——什麼紀錄都不留，就當這些東西從來不存在。',
      succRate: 0.5,
      costG: 150,
      succ: { bm: 'all' },
    ...
    2: {
      type: 'X',
      label:
        '請法律團隊進來做合規審查，挑出有風險的檔案依法處理——貴，但至少有律師背書，不是你一個人扛。',
      succRate: 0.9,
      costG: 50,
      succ: { bm: 1 },
      fail: { loss: 50 },
    ...
    3: {
      type: 'X',
      label:
        '聘資深律師幫這批資料寫一套說明書，把它的法律性質從燙手山芋變成正當資產——用文件把故事說圓。',
      succRate: 0.9,
      costG: 80,
      succ: { bm: 1 },
      fail: { loss: 50 },
  ...
  'E-02': {
    ...
    1: {
      type: 'Z',
      label:
        '透過事務所安排一場「私人調解」，給他一筆豐厚的補償——條件是上了法庭，說對公司有利的話。',
      succRate: 0.5,
      costG: 200,
      succ: { bm: 'all' },
    ...
    3: {
      type: 'Z',
      label: '一邊讓律師在法庭上全力攻擊他的誠信，一邊安排他去海外「出差」，拖到出庭日期過了再說。',
      succRate: 0.5,
      costG: 300,
      succ: { bm: 'all' },
  ...
  'E-03': {
    ...
    description:
      '技術主管悄悄拉你到旁邊說：系統後台把過去幾次連線紀錄都存下來了，時間、IP 位址、存取內容，一清二楚。他問你要怎麼處理，你看著那串紀錄，心裡很清楚這不是技術問題，是燙手山芋。',
    1: {
      type: 'X',
      label:
        '請技術律師幫公司訂一套「資料保留政策」，紀錄依法保存到期之後正常刪除——合規流程走完，刪得光明正大。',
      succRate: 0.9,
      costG: 50,
      succ: { bm: 1 },
      fail: { loss: 50 },
    ...
    3: {
      type: 'X',
      label:
        '找律師以「系統安全性維護」為由，把現有紀錄做加密處理——紀錄還在，但沒有人看得懂在寫什麼。',
      succRate: 0.9,
      costG: 80,
      succ: { bm: 1 },
      fail: { loss: 50 },
  ...
  'E-04': {
    ...
    1: {
      type: 'Z',
      label:
        '透過事務所牽線，找到一位辦事「靈活」的公證人，塞一筆急件費，請他對日期和內容睜一隻眼閉一隻眼。',
      succRate: 0.5,
      costG: 250,
      succ: { bm: 'all' },
  ...
  'E-05': {
    ...
    1: {
      type: 'X',
      label: '律師建議你主動出擊：如實申報、補繳欠稅，在調查介入前談好認罪協商。',
      succRate: 0.9,
      costG: 70,
      succ: { bm: 1 },
      fail: { loss: 50 },
  ...
  'E-06': {
    ...
    1: {
      type: 'Z',
      label: '以「資安升級」為名，把整台舊伺服器硬體替換掉，原本的設備直接銷毀。',
      succRate: 0.5,
      costG: 200,
      succ: { bm: 'all' },
      fail: { special: 'sue', lawCaseIds: ['E-01-1'] },
