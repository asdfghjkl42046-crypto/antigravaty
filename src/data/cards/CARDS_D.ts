...
export const CARDS_D: Record<string, Card> = {
  'D-01': {
    ...
    1: {
      type: 'C',
      label: '私下聯繫另外兩家有資格的對手，三方說好標價、分配利潤——表面上是競爭，實際上你們早就喬好了誰要得標。',
      succRate: 0.75,
    ...
    3: {
      type: 'B',
      label: '調查過程中發現對手的實績證明是偽造的，決定不淌這個渾水——直接向公共工程委員會舉報，讓評審自己去查。',
      rp: 8,
      special: 'skip_next',
    },
  },
  'D-02': {
    ...
    1: {
      type: 'A',
      label: '讓法務部門主導，照招標書每一項細節準備——獲利空間小一點沒關係，這種長期合約用實力拿才穩。',
      succRate: 0.9,
      succ: { g: 100, rp: 2 },
      fail: {},
    },
    2: {
      type: 'C',
      label: '赴約，喝了茶，對方說只要贊助他指定協會的「活動經費」，綜合評選你會拿到最高分——就這樣，不複雜。',
      succRate: 0.75,
    ...
    3: {
      type: 'A',
      label: '技術長提議引進最新低碳工法投標——成本高一點、獲利少一點，但在能源轉型的風口上，這個亮點可能比價格更有說服力。',
      succRate: 0.9,
      succ: { g: 100, rp: 2 },
      fail: {},
    },
  ...
  'D-03': {
    ...
    1: {
      type: 'B',
      label: '把對手和評委私下餐敘的證據整理好，寄去教育部廉政信箱——不投標了，但也不讓這件事就這樣過去。',
      rp: 8,
      special: 'skip_next',
    },
    2: {
      type: 'C',
      label: '對手有人脈，你也可以有——找一個跟特定政黨關係好的人，付他「政治顧問費」，請他幫你把評選這關打通。',
      succRate: 0.75,
    ...
    3: {
      type: 'A',
      label: '不管場外的事，把教材內容和使用體驗做到最好，展示會上直接讓基層教師和審查官員看到差距——用產品說話。',
      succRate: 0.9,
      succ: { g: 100, rp: 2 },
      fail: {},
    },
  ...
  'D-04': {
    ...
    1: {
      type: 'B',
      label: '整理好對手技術造假的證據，正式向主管機關書面舉報，然後退出這次競標——乾淨處理，但這個案子你就放掉了。',
      rp: 8,
      special: 'skip_next',
    },
    2: {
      type: 'B',
      label: '把標案規格疑似綁標的問題直接在記者會上公開爆料，讓媒體去追——聲量大、效果快，但你也會變成所有人的眼中釘。',
      rp: 8,
      special: 'skip_next',
    },
    3: {
      type: 'C',
      label: '找一個跟你關係好的民意代表，把標案規格偷偷改成只有你的軟體才能達標的條件——對手在綁標，你也可以。',
      succRate: 0.75,
  ...
  'D-06': {
    ...
    1: {
      type: 'C',
      label: '安排一趟「海外考察」，承辦人全家的機票住宿通通算進公關費用——費用公司出，標價怎麼調，大家心裡有數。',
      succRate: 0.75,
    ...
    2: {
      type: 'A',
      label: '預算就是預算，工法調整一下、視覺效果打點折扣——照規矩做，把能省的成本省出來，讓數字說話。',
      succRate: 0.9,
      succ: { g: 100, rp: 2 },
      fail: {},
    },
