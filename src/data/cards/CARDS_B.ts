...
export const CARDS_B: Record<string, Card> = {
  'B-01': {
    ...
    1: {
      type: 'A',
      label: '叫他先把原公司的事情處理乾淨，競業條款到期再來談——好的人才值得等，不需要用這種方式開始。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
    2: {
      type: 'C',
      label: '開出高額技術股權，但條件是他離職前要把原公司的核心代碼庫和客戶清單帶過來——當作「入場券」。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['刑法/背信罪', '妨害電腦使用罪'],
    ...
    3: {
      type: 'C',
      label: '在他正式離職前安排幾次「技術交流」，全程錄音——以後萬一他想反悔或獅子大開口，你手上有籌碼。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['妨害秘密罪', '強制罪'],
  ...
  'B-02': {
    ...
    1: {
      type: 'A',
      label: '照政府規定簽實習合約、保勞健保、加班給錢，再派個導師帶——麻煩是麻煩了點，但這樣留下來的人才是真的想跟你走的。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
    2: {
      type: 'A',
      label: '從裡面挑幾個最亮眼的，開一個讓他們驚喜的正職薪水，直接把人從實習轉正——搶先一步，不讓對手有機會接觸。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
    3: {
      type: 'A',
      label: '先讓法務把合約從頭到尾審一遍，所有流程攤在陽光下——聽起來最無聊，但日後最不會有人來找麻煩。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
  ...
  'B-03': {
    ...
    1: {
      type: 'C',
      label: '談好條件，入職第一週就要他交出前公司的完整客戶名單，順便用他還沒登出的帳號盯著競品的最新動態。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['個資法', '妨害電腦使用罪'],
    ...
    3: {
      type: 'C',
      label: '薪水照他說的給，但合約裡塞一份天價違約金協議和超時工作條款——讓他拿了錢也跑不掉，乖乖把所有資源都榨乾再說。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['勞基法', '強制罪'],
  ...
  'B-04': {
    ...
    1: {
      type: 'B',
      label: '私下接觸那幾個快跑的工程師，告訴他們跳過來有「搬家費」——但條件是帶著正在開發的產品原型圖一起來。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['營業秘密法'],
    ...
    2: {
      type: 'A',
      label: '辦幾場技術交流聚會，讓自家工程師去跟對方混熟——朋友聊天嘛，順便摸清楚對方的技術方向和人才狀況。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
  ...
  'B-05': {
    ...
    1: {
      type: 'B',
      label: '讓他們簽「承攬合約」而不是勞動合約——加班費就不用算了，再塞一條提早離職要賠三個月薪水的條款，想走也走不了。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['假承攬真僱傭', '勞基法'],
    ...
    2: {
      type: 'B',
      label: '直接上996，加班費不給，但發一堆股票期權當補償——反正公司真的上市的話，這些期權比加班費值錢多了。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['勞基法/以多報少'],
    ...
    3: {
      type: 'A',
      label: '訂清楚KPI、給彈性工時，表現真的不行的就依法輔導或資遣——麻煩，但至少你不會哪天被集體申訴。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
  ...
  'B-06': {
    ...
    1: {
      type: 'B',
      label: '讓我先進你名下一間不相關的子公司，薪水用「顧問費」名義發——人在你這裡，但帳面上跟競業條款沒有關係。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['無效競業禁止'],
    ...
    2: {
      type: 'B',
      label: '在海外開個人頭公司跟他簽約，研發獎金直接打進他的海外帳戶——錢出去了，但帳面上查不到你們之間有任何關係。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['商業背信'],
    ...
    3: {
      type: 'B',
      label: '暗中金援他去告前東家勞資糾紛，對方忙著應付官司、人心渙散——等時機到了，再光明正大地把他接過來。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['公平交易法', '誹謗罪'],
  ...
  'B-08': {
    ...
    1: {
      type: 'B',
      label: '把薪水拆成底薪加一堆津貼，津貼不計入投保薪資——對員工說這是「讓你實領更多」，對會計說這是「結構優化」。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['勞基法', '以多報少'],
    ...
    2: {
      type: 'C',
      label: '新人入職前先簽一份「自願放棄投保」的切結書，勞健保換成一個便宜的團體意外險——不簽就不用來了應。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['勞基法/以多報少', '強制罪'],
  ...
  'B-09': {
    ...
    1: {
      type: 'C',
      label: '全面改成承攬制，KPI沒達標就按比例扣錢，勞保健保通通不用給——合約上寫清楚，簽了就是他們自己同意的。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['假承攬真僱傭', '勞基法'],
    ...
    2: {
      type: 'A',
      label: '維持正規僱傭關係，但導入彈性排班和優於法規的福利——用真的好待遇留人，比玩合約文字遊戲划算。',
      succRate: 0.9,
      succ: { ip: 15, rp: 2 },
      fail: { ip: 0 },
    },
    3: {
      type: 'B',
      label: '專案型的人力改成短期承攬，核心員工還是給正職福利——灰色地帶走一走，至少不是全部壓上去賭。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['假承攬真僱傭'],
  ...
  'B-10': {
    ...
    1: {
      type: 'C',
      label: '私下找人傳話給他，說如果他繼續待在那個專案，你會把他在職時的考績紀錄全部公開——讓他自己衡量值不值得。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['妨害名譽罪', '強制罪'],
    ...
    2: {
      type: 'B',
      label: '協議有沒有效是一回事，先發一封措辭強硬的律師函給對方公司——就算告不贏，也能讓他們的開發節奏亂掉幾個月。',
      ip: 30,
      rp: -8,
      bm: 1,
      tags: ['無效競業禁止'],
    ...
    3: {
      type: 'C',
      label: '讓HR發一份措辭嚴厲的公開聲明，點名任何竊取商業機密的行為都會追究到底——雖然手上其實沒什麼證據。',
      ip: 60,
      rp: -20,
      bm: 3,
      tags: ['公平交易法', '妨害名譽罪'],
