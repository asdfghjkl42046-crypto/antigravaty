...
export default function AlignmentTool({ 
  ...
  // 輔助函數：將當前狀態推入歷史堆疊
  const pushToHistory = useCallback((newElements: Record<string, AlignmentElement>) => {
    setHistory(prev => {
  ...
  const updateElement = useCallback((id: string, updates: Partial<AlignmentElement>, finalize = false) => {
    const newElements = {
        ...elements,
      ...
      {Object.entries(elements).map(([id, el]) => (
        ...
        >
          {renderElement ? (
            ...
            <div className={`flex flex-col items-center justify-center text-center px-1 transition-opacity duration-300 ${id === selectedId ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="space-y-4 max-h-[80dvh] overflow-y-auto pr-1 custom-scrollbar text-white">
          ...
          {selectedElement && (
            <div className="space-y-4">
                ...
                <div className="pt-2 border-t border-white/5">
                  <label className="text-[7px] text-blue-400 font-bold mb-1 block">文字內容 (LABEL)</label>
                  <input 
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input type="number" step="0.1" title="垂直位置門檻 (Top %)" value={selectedElement.top.toFixed(1)} onChange={(e) => updateElement(selectedId!, { top: parseFloat(e.target.value) }, true)} className="w-full bg-black/40 px-2 py-1.5 text-[10px] rounded border border-white/5 font-mono text-white" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 block font-bold">LEFT (%)</label>
                  <input type="number" step="0.1" title="水平位置門檻 (Left %)" value={selectedElement.left.toFixed(1)} onChange={(e) => updateElement(selectedId!, { left: parseFloat(e.target.value) }, true)} className="w-full bg-black/40 px-2 py-1.5 text-[10px] rounded border border-white/5 font-mono text-white" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 block font-bold">WIDTH (%)</label>
                  <input type="number" step="0.1" title="元件寬度百分比 (Width %)" value={selectedElement.width.toFixed(1)} onChange={(e) => updateElement(selectedId!, { width: parseFloat(e.target.value) }, true)} className="w-full bg-black/40 px-2 py-1.5 text-[10px] rounded border border-white/5 font-mono text-white" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 block font-bold">HEIGHT (%)</label>
                  <input type="number" step="0.1" title="元件高度百分比 (Height %)" value={selectedElement.height.toFixed(1)} onChange={(e) => updateElement(selectedId!, { height: parseFloat(e.target.value) }, true)} className="w-full bg-black/40 px-2 py-1.5 text-[10px] rounded border border-white/5 font-mono text-white" />
                </div>
              </div>
              ...
              <div>
                <label className="text-[9px] text-slate-500 flex items-center justify-between mb-1">
                  <span className="flex items-center gap-1 font-bold"><Type className="w-3 h-3 text-[#22D3EE]" /> FONT SIZE ({selectedElement.fontSize || 14}px)</span>
                </label>
                <div className="flex items-center gap-3">
                  <input 
                    ...
                    onChange={(e) => updateElement(selectedId!, { fontSize: parseInt(e.target.value) }, true)}
              ...
              <div>
                <label className="text-[9px] text-slate-500 flex items-center justify-between mb-1">
                  <span className="flex items-center gap-1 font-bold"><CircleDashed className="w-3 h-3 text-emerald-400" /> ROUNDNESS ({Math.round(selectedElement.radius || 0)}px)</span>
                </label>
                <input 
                  type="range" min="0" max="100" 
                  title="圓角半徑滑桿 (Border Radius)"
                  value={selectedElement.radius || 0} 
                  onChange={(e) => updateElement(selectedId!, { radius: parseInt(e.target.value) }, true)}
          ...
          <div className="pt-4 border-t border-white/10 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-1">
               <p className="text-[8px] text-slate-500 text-center">Ctrl+Z: 撤銷 • Backspace: 刪除元件</p>
