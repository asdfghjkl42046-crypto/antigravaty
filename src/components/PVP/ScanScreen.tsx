...
export default function ScanScreen({ onBack, onEndTurn, onNavigate }: ScanScreenProps): React.ReactElement {
  ...
  const startScanning = async () => {
    ...
    if (typeof window !== 'undefined' && !window.isSecureContext) {
      setStatus({
        type: 'error',
        msg: SYSTEM_STRINGS.ERRORS.SECURE_CONTEXT_REQUIRED,
      });
      return;
    }
    ...
    try {
      ...
      setStatus({ type: 'error', msg: SYSTEM_STRINGS.ERRORS.CAMERA_START_FAIL });
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center space-y-4">
        <div className="relative w-64 h-64 border-2 border-slate-800 rounded-[32px] overflow-hidden bg-black/40 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          {!isCameraActive && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-slate-900/80 backdrop-blur-sm">
              ...
              >
                {SYSTEM_STRINGS.SCAN.START_CAMERA}
              </button>
            </div>
          )}
        ...
        <div className="w-full max-w-xs space-y-4">
          <div className="bg-slate-900/60 border border-white/10 rounded-3xl p-5 shadow-inner overflow-hidden">
            <div className="flex items-center space-x-2 mb-3">
              ...
              <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">
                {SYSTEM_STRINGS.SCAN.MANUAL_INPUT_LABEL}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <input
                type="text"
                placeholder={SYSTEM_STRINGS.SCAN.INPUT_PLACEHOLDER}
                value={manualCode}
              ...
              <button
                ...
                title={SYSTEM_STRINGS.SCAN.DECODE_SYNC}
          ...
          {onEndTurn && (
            <button
              onClick={onEndTurn}
              title={SYSTEM_STRINGS.UI_LABELS.END_TURN}
              aria-label="End turn"
